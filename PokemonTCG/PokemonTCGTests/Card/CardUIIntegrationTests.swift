...
class CardUIIntegrationTests: XCTestCase {
    func test_loadFeedActions_requestCardsFromLoader() {
        ...
        loader.completeCardLoading(at: 1)
        sut.simulateUserInitiatedReload()
    ...
    func test_loadCardsCompletion_rendersErrorMessageOnErrorUntilNextReload() {
        let (sut, loader) = makeSUT()
        
        sut.loadViewIfNeeded()
        XCTAssertEqual(sut.errorMessage, nil)
        
        loader.completeCardLoadingWithError(at: 0)
    ...
    private func makeSUT(
        ...
        let sut = CardListUIComposer.cardListComposedWith(
            ...
            priceFormatter: .priceFormatter) { _ in }
